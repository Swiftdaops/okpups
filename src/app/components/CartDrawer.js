...
function buildWhatsAppMessage({ name, orderId, items, subtotal }) {
  const lines = [];
  lines.push("Hi OKPUPS, I want to pay for the items I ordered.");
  ...
  for (const it of items) {
    ...
    lines.push(`- ${it.name} (${it._type}) x${qty} — ${price}`);
  }
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.aside
            className="fixed right-0 top-0 z-50 h-full w-[min(24rem,92vw)] bg-white shadow-xl border-l flex flex-col"
